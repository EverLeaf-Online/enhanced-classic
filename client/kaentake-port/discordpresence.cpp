    ...
    while (incoming.bytes.size() >= sizeof(FrameHeader)) {
        ...
        if (opcode == Opcode::Ping) {
            ...
            if (StringFieldEquals(payload, "evt", "ERROR")) {
                DEBUG_MESSAGE("Discord Rich Presence request rejected");
                return false;
            }
...
DWORD WINAPI WorkerProc(LPVOID) {
    while (!gStopRequested.load()) {
        ...
        while (!gStopRequested.load()) {
            ...
            if (!announced) {
                DEBUG_MESSAGE("Discord Rich Presence activity acknowledged");
                announced = true;
            }
...
void Start() {
    ...
    if (!worker) {
        gRunning.store(false);
        DEBUG_MESSAGE("Discord Rich Presence worker unavailable");
        return;
    }
    CloseHandle(worker);
}
...
void AttachEverLeafDiscordPresenceMod() {
