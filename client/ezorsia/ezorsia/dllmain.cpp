// dllmain.cpp : EverLeaf v83 client proxy bootstrap.
#include "stdafx.h"
...
// These are stable entry bytes from EverLeaf's pinned GMS v83 client after the
// Themida-protected image has unpacked. Waiting on several independent owners is
// safer than letting the first individual hook spin forever on one byte.
...
bool InstallEarlyHooks() {
    // Detours are deliberately installed on the bootstrap worker instead of in
    // DllMain. This avoids doing transaction/loader-sensitive work while the
...
// but the Client v2 preflight prevents normal startup from entering them early.
void MainFunc() {
    bool hooksOk = true;
    ...
    // The old MyGetProcAddress replacement is intentionally not installed. It is
    // a pass-through in EverLeaf and was the earliest serial unpack wait, so it
    // added startup-failure surface without providing production behavior.
    ...
    // Tracking-only hooks that were inherited from the reverse-engineering
    // template are intentionally omitted from the production v2 bootstrap.
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    ...
    case DLL_PROCESS_ATTACH: {
        ...
        MainMain::mainTHread = OpenThread(THREAD_SUSPEND_RESUME | THREAD_QUERY_INFORMATION,
                                          FALSE,
                                          GetCurrentThreadId());
    ...
    case DLL_PROCESS_DETACH:
        ...
        // already reclaims these resources, and running provider/destructor work
        // during loader-lock teardown was a known crash/null-deref risk.
