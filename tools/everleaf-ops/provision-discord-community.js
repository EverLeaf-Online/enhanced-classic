...
(async () => {
  ...
  if (created.get("server-status")?.created) await post(created.get("server-status").channel.id, "Server availability and planned maintenance notices will appear here. Live status is also available at https://everleafms.duckdns.org.");
