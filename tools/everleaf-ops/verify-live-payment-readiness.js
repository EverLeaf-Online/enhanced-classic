...
(async () => {
  ...
  const stripeReady = (stripe.data || []).some((item) => item.url === "https://everleafms.duckdns.org/webhooks/stripe" && item.status === "enabled" && item.enabled_events.includes("checkout.session.completed"));
  ...
  const paypalReady = (paypal.webhooks || []).some((item) => item.id === values.PAYPAL_LIVE_WEBHOOK_ID && item.url === "https://everleafms.duckdns.org/webhooks/paypal" && item.event_types.some((event) => event.name === "PAYMENT.CAPTURE.COMPLETED"));
