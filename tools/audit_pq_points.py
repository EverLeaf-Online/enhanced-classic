#!/usr/bin/env python3
"""Static readiness checks for EverLeaf PQ Points, rewards, and economy guards."""
...
def main() -> None:
    ...
    # Player shops and hired merchants accept a bundle count supplied by the
    # client. Multiplication must happen in a wide type before narrowing to the
    # protocol's signed-short item quantity, and price arithmetic must not use
    # float (which can round large transactions into an undercharge).
    merchant_guard_tokens = [
        "HIRED_MERCHANT_PATH",
        "PLAYER_SHOP_PATH",
        "long requestedUnits = (long) pItem.getItem().getQuantity() * quantity;",
        "requestedUnits > Short.MAX_VALUE",
        "long requestedPrice = (long) pItem.getPrice() * quantity;",
        "requestedPrice < 0",
        "Math.min(requestedPrice, Integer.MAX_VALUE)",
    ]
    for token in merchant_guard_tokens:
        if token not in transform:
            fail(f"Merchant/shop transform is missing arithmetic guard: {token}")

    if "Math.min((float) pItem.getPrice() * quantity" in transform:
        fail("Merchant/shop transform must not retain float transaction arithmetic")
    ...
    print("[PASS] PQ Points/reward/economy audit")
    ...
    print("       merchant arithmetic=long multiplication + signed-short quantity guard")
