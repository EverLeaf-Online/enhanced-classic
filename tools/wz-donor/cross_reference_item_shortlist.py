...
BASELINE_PATHS = (
    "scripts", "sql", "src", "wz/Quest.wz", "wz/Map.wz", "wz/Mob.wz", "wz/Npc.wz", "wz/Reactor.wz",
)
...
def scan_references(ids: list[str], root: Path, relative_roots: tuple[str, ...], sample_limit: int) -> dict[str, dict]:
    results = {content_id: {"referenceCount": 0, "fileCount": 0, "byRoot": Counter(), "samples": []} for content_id in ids}
    if not ids:
        return results
    ...
    for path in iter_text_files(root, relative_roots):
        ...
        for content_id, count in Counter(matches).items():
...
def build_report(shortlist: dict, donor_root: Path, baseline_root: Path, sample_limit: int = 12) -> dict:
    ...
    rows, coupled_count, quest_coupled_count = [], 0, 0
    for candidate in candidates:
        content_id = str(candidate["contentId"])
        donor_result, baseline_result = donor[content_id], baseline[content_id]
        coupled = donor_result["referenceCount"] > 0 or baseline_result["referenceCount"] > 0
        quest_coupled = donor_result["byRoot"].get("Quest.wz", 0) > 0
        coupled_count += int(coupled)
        quest_coupled_count += int(quest_coupled)
        rows.append({
            "contentId": content_id, "name": candidate.get("name"),
            "donorReferences": donor_result, "baselineReferences": baseline_result,
            "hasAnyCrossReference": coupled, "hasDonorQuestReference": quest_coupled,
            "approved": False, "importAllowed": False,
        })
    return {
        "schemaVersion": 1, "kind": "review-only-item-cross-reference-report", "donorId": shortlist.get("donorId"),
        "candidateCount": len(rows), "crossReferencedCandidateCount": coupled_count,
        "donorQuestReferencedCandidateCount": quest_coupled_count, "uncoupledCandidateCount": len(rows) - coupled_count,
        "scanScope": {"donorRoots": list(DONOR_FAMILIES), "baselineRoots": list(BASELINE_PATHS), "matchRule": "numeric token boundary; review evidence only"},
        "candidates": rows, "approved": False, "automaticImport": False,
...
def main() -> int:
    ...
    parser.add_argument("shortlist", type=Path); parser.add_argument("--donor", type=Path, required=True)
    parser.add_argument("--baseline", type=Path, required=True); parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--sample-limit", type=int, default=12); args = parser.parse_args()
    ...
    for row in report["candidates"]:
        if row["hasAnyCrossReference"]:
            print(f"{row['contentId']}\t{row.get('name') or ''}\tdonor={row['donorReferences']['referenceCount']}\tbaseline={row['baselineReferences']['referenceCount']}\tquest={str(row['hasDonorQuestReference']).lower()}")
    return 0
