...
static void CopyImage(WzFile target,WzFile donor,int id,bool allowReplace,List<object> changes){var name=id+".img";var ds=FindImage(donor.WzDirectory,name)??throw new InvalidDataException($"Donor missing {name}");var ts=FindImage(target.WzDirectory,name);string action;
    ...
    var parent=EnsureDirs(target.WzDirectory,ds.dirs);parent.AddImage(ds.image.DeepClone());changes.Add(new{contentId=id,action,directory=string.Join("/",ds.dirs)});
}
...
var digestEvidence=new List<object>();foreach(var family in new[]{("Map.wz",maps),("Mob.wz",mobs),("Npc.wz",npcs)}){using var donor=OpenDonor(Path.Combine(donorDir,family.Item1));using var output=OpenTarget(Path.Combine(outputDir,family.Item1));foreach(var id in family.Item2){var name=id+".img";var di=FindImage(donor.WzDirectory,name)??throw new InvalidDataException($"Donor validation missing {name}");var oi=FindImage(output.WzDirectory,name)??throw new InvalidDataException($"Output validation missing {name}");var dd=ImageDigest(di.image);var od=ImageDigest(oi.image);if(dd!=od)throw new InvalidDataException($"Reparsed image digest mismatch {family.Item1}:{id}");digestEvidence.Add(new{family=family.Item1,contentId=id,donorDigest=dd,outputDigest=od});}}
