...
- `tools/audit_world_links.py` for cross-map target-portal review and exact duplicate spawn/link records.
...
Missing target maps, invalid WZ object references, parse failures, and explicitly validated active progression links are appropriate release gates. The v83 baseline contains hundreds of legacy cross-map target-portal name mismatches, so exact target-name mismatches remain review findings until the route is proven active and player-facing. Generic `sp`/`portal` fallbacks are not treated as literal target-name contracts.
