...
# Match only APIs where the first numeric argument unambiguously represents a map.
MAP_CALL = re.compile(
...
def excluded_map(map_id: int) -> bool:
    # Ereve / Knights of Cygnus content is explicitly outside EverLeaf's scope.
...
def scan_file(path: Path, maps: set[int]) -> tuple[int, list[Finding]]:
    ...
    for number, line in enumerate(text.splitlines(), start=1):
        # Comments frequently contain stale example IDs. Ignore full-line comments;
        # inline comments remain safe because matches occur before the comment text
        # in normal script syntax.
        stripped = line.lstrip()
        if stripped.startswith("//") or stripped.startswith("/*") or stripped.startswith("*"):
            continue

        matches = list(MAP_CALL.finditer(line))
        # getMap() may appear after an object chain not covered by the common script
        # manager prefixes. Add those without double-counting an existing match.
