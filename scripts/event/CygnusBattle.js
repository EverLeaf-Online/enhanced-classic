...
var minMapId = 271040100, maxMapId = 271040100;
var eventTime = 60;
const maxLobbies = 1;
...
function spawnTuned(eim, id, x, y, hp, mp, exp, watk, matk, level) {
    var OverrideMonsterStats = Java.type('server.life.OverrideMonsterStats');
    var Point = Java.type('java.awt.Point');
    var mob = eim.getMonster(id);
    if (mob == null) throw "Missing Cygnus mob " + id;
    mob.setOverrideStats(new OverrideMonsterStats(hp, mp, exp, watk, matk, 900, 900, level));
    mob.disableDrops();
    eim.getMapInstance(entryMap).spawnMonsterOnGroundBelow(mob, new Point(x, y));
}

function startEncounter(eim) {
    if (eim.isEventCleared()) return;
    eim.setIntProperty("everleafPhase", 1); eim.setIntProperty("everleafPhaseKills", 0);
    eim.dropMessage(5, "[EverLeaf] The Chief Knights have taken the field.");
    var xs = [-520,-260,0,260,520];
    for (var i=0;i<5;i++) spawnTuned(eim, 8850000+i, xs[i], 112, 60000000, 50000, 1000000, 1900, 1850, 185);
}

function startEliteKnights(eim) {
    if (eim.isEventCleared()) return;
    eim.setIntProperty("everleafPhase", 2); eim.setIntProperty("everleafPhaseKills", 0);
    eim.dropMessage(5, "[EverLeaf] The Chief Knights return with their full strength!");
    var xs = [-520,-260,0,260,520];
    for (var i=0;i<5;i++) spawnTuned(eim, 8850005+i, xs[i], 112, 90000000, 65000, 1500000, 2350, 2250, 188);
}

function startShinsoo(eim) {
    if (eim.isEventCleared()) return;
    eim.setIntProperty("everleafPhase", 3); eim.setIntProperty("everleafPhaseKills", 0);
    eim.dropMessage(5, "[EverLeaf] Shinsoo descends to defend the Empress. Defeat it before it withdraws!");
    spawnTuned(eim, 8850010, 0, 112, 120000000, 50000, 0, 2500, 2400, 190);
...
function startCygnus(eim) {
    ...
    eim.setIntProperty("everleafPhase", 4); eim.setIntProperty("everleafPhaseKills", 0);
    eim.dropMessage(5, "[EverLeaf] Fallen Cygnus has appeared. This is the final phase!");
    spawnTuned(eim, 8850011, 0, 112, 500000000, 90000, 25000000, 2850, 2750, 195);
}
...
function changedMap(eim, player, mapid) {
    if (mapid < minMapId || mapid > maxMapId) { eim.unregisterPlayer(player); if (eim.getPlayerCount()==0 && !eim.isEventCleared()) end(eim); }
}
...
function end(eim) { var p=eim.getPlayers(); for(var i=p.size()-1;i>=0;i--) playerExit(eim,p.get(i)); eim.dispose(); }
...
function finishPlayer(eim, player) {
    ...
    try {
        ...
        var InventoryManipulator=Java.type('server.InventoryManipulator');
        var Instant=Java.type('java.time.Instant');
        var Long=Java.type('java.lang.Long');
        ...
        if (rare.itemId()!=0) {
            if (player.canHold(rare.itemId())) {
                InventoryManipulator.addById(player.getClient(),rare.itemId(),1,"Fallen Cygnus weekly clear",-1);
                player.dropMessage(5,"[EverLeaf] Fallen Cygnus weekly rare reward received!");
            } else {
                player.dropMessage(5,"[EverLeaf] Your rare Cygnus reward could not fit in inventory. Contact a GM with this clear time.");
            }
        } else player.dropMessage(5,"[EverLeaf] Weekly Fallen Cygnus clear recorded. No rare scroll rolled this week.");
...
function monsterKilled(mob,eim) {
    var id=mob.getId(), phase=eim.getIntProperty("everleafPhase");
    if (phase==1 && id>=8850000 && id<=8850004) {
        var k=eim.getIntProperty("everleafPhaseKills")+1; eim.setIntProperty("everleafPhaseKills",k); if(k>=5)eim.schedule("startEliteKnights",2500);
    } else if (phase==2 && id>=8850005 && id<=8850009) {
        var k2=eim.getIntProperty("everleafPhaseKills")+1; eim.setIntProperty("everleafPhaseKills",k2); if(k2>=5)eim.schedule("startShinsoo",2500);
    } else if (phase==3 && id==8850010) {
        eim.schedule("startCygnus",2500);
    } else if (phase==4 && id==8850011) {
