        ...
        public StatEffect getAttackEffect(Character chr, Skill theSkill) {
            ...
            if (display > 80) { //Hmm
                if (!mySkill.getAction()) {
                    String detail = "WZ Edit; adding action to a skill: " + display + " skill=" + skill;
                    AutobanFactory.FAST_ATTACK.alert(chr, detail);
                    AntiCheatService.flag(chr, "INVALID_SKILL_ACTION", detail, true);
                    return null;
                }
            }
        ...
        try {
            ...
            if (attack.skill != 0) {
                ...
                if (attack.numAttacked > mobCount) {
                    String detail = "Skill: " + attack.skill + "; Count: " + attack.numAttacked + " Max: " + attackEffect.getMobCount();
                    AutobanFactory.MOB_COUNT.alert(player, detail);
                    AntiCheatService.flag(player, "MOB_COUNT", detail, true);
                    return;
                }
            }
            if (!player.isAlive()) {
                    ...
                    if (attack.skill != 0) {
                        if (attackEffect.getFixDamage() != -1) {
                            if (totDamageToOneMonster != attackEffect.getFixDamage() && totDamageToOneMonster != 0) {
                                String detail = "damage=" + totDamageToOneMonster + " expected=" + attackEffect.getFixDamage() + " skill=" + attack.skill;
                                AutobanFactory.FIX_DAMAGE.alert(player, detail);
                                AntiCheatService.flag(player, "FIXED_DAMAGE", detail, true);
                                return;
                            }
    ...
    protected AttackInfo parseDamage(InPacket p, Character chr, boolean ranged, boolean magic) {
        ...
        for (int i = 0; i < ret.numAttacked; i++) {
            ...
            for (int j = 0; j < ret.numDamage; j++) {
                ...
                // Values over 5x the server envelope are treated as high-confidence packet edits.
                if (damage > maxWithCrit * 5) {
                    String detail = "DMG: " + damage + " MaxDMG: " + maxWithCrit + " SID: " + ret.skill + " MobID: " + (monster != null ? monster.getId() : "null") + " Map: " + chr.getMap().getMapName() + " (" + chr.getMapId() + ")";
                    AutobanFactory.DAMAGE_HACK.addPoint(chr.getAutobanManager(), detail);
                    AntiCheatService.flag(chr, "DAMAGE_HACK_EXTREME", detail, true);
                    ret.invalid = true;
                }
                ...
                if(effect != null) {
                    ...
                    if (ret.numDamage > maxattack) {
                        String detail = "Too many lines: " + ret.numDamage + " Max lines: " + maxattack + " SID: " + ret.skill + " MobID: " + (monster != null ? monster.getId() : "null") + " Map: " + chr.getMap().getMapName() + " (" + chr.getMapId() + ")";
                        AutobanFactory.DAMAGE_HACK.addPoint(chr.getAutobanManager(), detail);
                        AntiCheatService.flag(chr, "DAMAGE_LINES", detail, true);
                        ret.invalid = true;
                    }
                }
